...
# TODO: 
# - [high] Automatically add file suffix when saving.  (ok done)
# - [high] Add polygon movement with arrow keys. (ok done)
...
#TODO: Herobot WANT TO DO
# Adding an image precessing program  to auto mark function. (ok done)
# Batch production of VOC data. (ok done)
# Batch rename pictures and other data,convert picture format function.
...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, filename=None):
        ...
        nextImg = action('nextImg', self.openNextImg,#self.openFile,
                'G', 'nextImg', u'Start next image')
        nextLif = action('nextLif', self.openNextLif,
                'F', 'nextLif', u'Start next Lif')
        drawPoint = action('DrawPoint', self.drawPoint,#self.openFile,
                'D', 'drawPoint', u'Draw point in image')
        ...
        edit = action('&Edit Label', self.editLabel,
                'E', 'edit', u'Modify the label of the selected polygon',
                enabled=False)
        ...
        self.actions = struct(save=save, saveAs=saveAs, open=open, nextImg=nextImg,nextLif=nextLif,drawpoint=drawPoint, close=close,
                ...
                fileMenuActions=(open,nextImg,nextLif,drawPoint,save,saveAs,close,quit),
                beginner=(), advanced=(),
        ...
        addActions(self.menus.file,
                (open, nextImg, nextLif, drawPoint, self.menus.recentFiles, save, saveAs, close, None, quit))
        ...
        self.actions.beginner = (
            open, save, None, nextImg, nextLif, drawPoint, create, copy, delete, None,
            zoomIn, zoom, zoomOut, fitWindow, fitWidth)

        self.actions.advanced = (
            open, nextImg, nextLif, drawPoint, save, None,
        ...
        size = settings.get('window/size', QSize(1920, 1080)) # changed by HT
    ...
    def popLabelListMenu(self, point):
        ...
        # point_txt = open("point.txt", 'w')
        # point_txt.write(str(point))
        # point_txt.close()
    ...
    def loadLabels(self, shapes):
        ...
        for label, points, line_color, fill_color in shapes:
            ...
            contours_txt = open("loadLabels_points.txt", 'a')
            contours_txt.write(str(points))
            contours_txt.close()
    ...
    def saveLabels(self, filename):
        ...
        def format_shape(s):
            ...
            # canvasShapes_txt = open("#points1018.txt", 'w')
            # canvasShapes_txt.write(str(s.points))
            # canvasShapes_txt.close()
            #
            # canvasShapes_txt = open("#point1019.txt", 'w')
            # canvasShapes_txt.write(str([(p.x(), p.y()) for p in s.points]))
            # canvasShapes_txt.close()
        ...
        if os.path.exists(edgeImgPath):
            ep = EdgePoint()
            edgePoints = ep.find_edge_counters(edgeImgPath,current_count-1)

        def format_edge_shape(edgePoints):
            temp = []
            for edgePoint in edgePoints:
                if len(edgePoint) < 2:
                    ...
                    temp.append(
                        dict(label=unicode(1),
                            line_color=None, \
                            fill_color=None, \
                            points=[(p[0][0],p[0][1]) for p in edgePoint]))
            return temp


        if self.autoSignState == False:
            shapes = [format_shape(shape) for shape in self.canvas.shapes]
        else:
            shapes = format_edge_shape(edgePoints)
    ...
    def loadFile(self, filename=None):
        ...
        if QFile.exists(filename):
            ...
            if self.labelFile:
                ...
                contours_out_txt = open("#selflabelFileshapes.txt",'w')
                contours_out_txt.write(str(self.labelFile.shapes))
    ...
    def openFile(self, _value=False):
        ...
        if filename:
            ...
            current_count = int(os.path.split(filename)[1].split('.')[0]) + 1
            # print current_count
            self.loadFile(filename)
    ...
    def openNextLif(self, _value=False):
        all_img_count = find_image_count(imgFilePath)
        if all_img_count == 0:
            return
        global current_count
        # countLifName = '{:0>6}.lif'.format(current_count-1)  # 故意current_count-1使得结果重复显示用于检查标记结果的。
        countLifName = '{:0>6}.lif'.format(current_count)
        print countLifName.replace('.lif', '')
        lifPath = os.path.join(imgFilePath, countLifName)
        print'现在处理第{}张图片,剩余{}张'.format(current_count + 1, (all_img_count - current_count - 1))
        if lifPath:
            self.loadFile(lifPath)
        current_count += 1

    def openCurrentLif(self, _value=False):
        all_img_count = find_image_count(imgFilePath)
        if all_img_count == 0:
            return
        global current_lif_count
        current_lif_count = current_count-1
        countLifName = '{:0>6}.lif'.format(current_lif_count)
        lifPath = os.path.join(imgFilePath, countLifName)
        if lifPath:
            self.loadFile(lifPath)
...
#                                         else self.saveFileDialog())
    def drawPoint(self,_value=False):
        # # 1. 创建新的label,虚拟画出来一个（creat) end
        # # 2. 将point画出来,保存为lif文件. end  (bug: 打开lif文件时处于画点的状态—已经修正)
        # # 3. 打开lif文件，标注并保存lif。
        # self.setBeginner()
        # self.setAdvanced()
        shape = Shape("Copy of %s" % 1 )
        self._beginner = False
        self.setCreateMode()
        self.addLabel(shape)
        self.autoSignState = True
        self.saveFile()
        self.setEditMode()
        self.openCurrentLif()
        self._beginner = True
...
#            self._saveFile(self.saveFileDialog())
    ...
    def saveFileDirect(self,filePath):
        # 文件保存路径为当前处理图片所在路径
        # 写入的文件名与当前处理的图片名相同
        countImgName = '{:0>6}.jpg'.format(current_count-1)
...
#打开软件的open文件选项指定或者此处设定
current_count = 0

#保证文件夹中文件格式为{:0>6}.jpg/bmp/lif
# imgFilePath = 'E:/clothData/2017_10_11/clothRed/source/jpg'
# imgFilePath = 'E:/clothData/2017_11_16/source/jpg'
# imgFilePath = 'E:/clothData/2017_10_11/clothRed/source/resize'
imgFilePath = '/home/herobot/data/redClothTest/source/jpg'

# edgeImgPath是imgFilePath中图片的缺陷检测结果图（FCN结果/或者传统算法的填充结果——二值图）
# edgeImgPath = 'E:/clothData/2017_10_11/clothRed/source/dete'
# edgeImgPath = 'E:/clothData/2017_11_16/source/dete'
edgeImgPath = '/home/herobot/data/redClothTest/source/dect'
